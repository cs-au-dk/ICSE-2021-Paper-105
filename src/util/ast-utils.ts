import {AssignmentPattern, AssignmentProperty, ArrayExpression, BlockStatement, CallExpression, ExportNamedDeclaration, ExpressionStatement, FunctionExpression, FunctionDeclaration, Identifier, ImportSpecifier, ImportDeclaration, MemberExpression, NewExpression, ObjectExpression, ObjectPattern, SimpleCallExpression, SimpleLiteral, ThisExpression, ArrowFunctionExpression, AssignmentExpression, SpreadElement, Program, Property, TemplateLiteral, VariableDeclarator,
  // @ts-ignore
...
export function isAnyCreateFunctionNode(node: any) {
...
export function isRequireCall(node: any): node is SimpleCallExpression {
  return isSimpleCallExpression(node) && isIdentifier(node.callee) && node.callee.name === 'require' &&
      node.arguments.length === 1;
}

export function isModuleImport(n : any): boolean {
...
export function isParenthesizedExpression(node: any): node is ParenthesizedExpression {
...
export function isAssignmentProperty(node: any): node is AssignmentProperty {
  return node && node.type === 'AssignmentProperty';
}
