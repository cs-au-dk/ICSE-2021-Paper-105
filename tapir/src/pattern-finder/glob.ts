...
export class GlobConstant implements GlobPattern {
    ...
    public matches(path: string, wildcardNum: number): GlobMatch[] {
        ...
        return globResults.map(globResult => new GlobMatch(this.constant + globResult.stringMatched, globResult.getWildcardMatches()));
    }
...
export function parseGlobPattern(pattern: string): GlobPattern {
    ...
    if (nextSpecialCharacter === GLOB_STAR) {
        ...
        const disjunctionPatterns = pattern.substring(specialCharacterIndex + STAR.length, pattern.indexOf("}"))
            .split(",").map(str => parseGlobPattern(str.trim()));
